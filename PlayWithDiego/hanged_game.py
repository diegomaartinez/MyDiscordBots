...
with open('./PlayWithDiego/reactions.json', 'r') as file:
    ...
    while wrong_attempts < max_attempts and "_" in display_word:
        try:
            ...
            if guess == word_to_guess:
                ...
                await game_message.add_reaction(random.choice(reactions['correct']))
                update_stats(interaction.guild.id, interaction.user.id, interaction.user.name, won=True, game=game)
                return
            ...
            await game_message.edit(embed=embed)
            await game_message.add_reaction(random.choice(reactions['incorrect']))
            update_stats(interaction.guild.id, interaction.user.id, interaction.user.name, won=False, game=game)
            return
